...
<div class="flex justify-between items-center h-full">
    ...
    <div class="items-center space-x-4 rtl:space-x-reverse horizental-box">
     ...
     <div class="main-menu">
                                <ul>
                                    <li class="{{ (request()->is('admin/dashboard')) ? 'active' : '' }} menuatas">
                                        <a href="{{ route('admin.menu.dashboard') }}" class="Navbar">
                                    ...
                                    <li class="{{(request()->is('admin/review')) ? 'active' : '' }} menuatas">
        ...
        <div class=" w-full">
            <button class="text-slate-800 dark:text-white focus:ring-0 focus:outline-none font-medium rounded-lg text-sm text-center inline-flex items-center" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                <div class="lg:h-8 lg:w-8 h-7 w-7 rounded-full flex-1 ltr:mr-[10px] rtl:ml-[10px]">
                    <img src="{{ Auth::guard('webadministration')->user()->foto_profil ? Auth::guard('webadministration')->user()->foto_profil : url('assets/images/avatar/av-1.svg') }}" alt="user" class="block w-full h-full object-cover rounded-full">
                </div>
                <span class="flex-none text-slate-600 dark:text-white text-sm font-normal items-center lg:flex hidden overflow-hidden text-ellipsis whitespace-nowrap">{{ Auth::guard('webadministration')->user()->nama }}</span>
...
@section('menu-mobile')
<div class="bg-white bg-no-repeat custom-dropshadow footer-bg dark:bg-slate-700 flex justify-around items-center backdrop-filter backdrop-blur-[40px] fixed left-0 bottom-0 w-full z-[9999] bothrefm-0 py-[12px] px-4 xl:hidden">
    <a href="{{route('admin.menu.dashboard')}}">
